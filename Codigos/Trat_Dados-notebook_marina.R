...
dados_completos <- read_delim(
  "DadosSIM_Homicidios_Femininos.csv",
  delim = ";", escape_double = FALSE, 
  col_types = cols(...1 = col_skip(),
                   HORAOBITO = col_time(format = "%H%M")),
  trim_ws = TRUE) %>% 
  mutate(
    # Normaliza e extrai o padrão LETRA + 2 DÍGITOS (ex.: X93, Y04)
...
dados_projecao <- read_excel("projecoes_2024_tab1_idade_simple.xlsx") %>% 
  pivot_longer(
    cols = matches("^20\\d{2}$"),
    names_to = "ano",
    values_to = "pop"
...
dados_prop <- read_excel("Tabela 9606.xlsx", skip = 5)
...
sim <- dados_completos %>%
  ...
  mutate(
    ...
    faixa_etaria = case_when(
      IDADEanos < 15 ~ "<15",
      IDADEanos >= 15 & IDADEanos <= 29 ~ "15 a 29",
      IDADEanos >= 30 & IDADEanos <= 49 ~ "30 a 49",
      IDADEanos == 50 ~ "49 a 50",
      IDADEanos > 50 & IDADEanos <= 64 ~ "50 a 64",
      TRUE ~ NA_character_
    ),
    faixa_etaria = factor(
      faixa_etaria,
      levels = c("<15", "15 a 29", "30 a 49", "49 a 50", "50 a 64"),
      ordered = F
    )
  ) %>% 
  mutate(
    fe_quinque = case_when(
      IDADEanos < 15 ~ "<15",
      IDADEanos >= 15 & IDADEanos <= 19 ~ "15 a 19",
      IDADEanos >= 20 & IDADEanos <= 24 ~ "20 a 24",
      IDADEanos >= 25 & IDADEanos <= 29 ~ "25 a 29",
      IDADEanos >= 30 & IDADEanos <= 34 ~ "30 a 34",
      IDADEanos >= 35 & IDADEanos <= 39 ~ "35 a 39",
      IDADEanos >= 40 & IDADEanos <= 44 ~ "40 a 44",
      IDADEanos >= 45 & IDADEanos <= 49 ~ "45 a 49",
      IDADEanos >= 50 & IDADEanos <= 54 ~ "50 a 54",
      IDADEanos >= 55 & IDADEanos <= 59 ~ "55 a 59",
      IDADEanos >= 60 & IDADEanos <= 64 ~ "60 a 64",
      TRUE ~ NA_character_
    ),
    fe_quinque = factor(
      fe_quinque,
      levels = c("<15", "15 a 19", "20 a 24", "25 a 29", "30 a 34", "35 a 39",
                 "40 a 44", "45 a 49", "50 a 54", "55 a 59", "60 a 64"),
      ordered = TRUE
    )
  ) %>% 
  mutate(
