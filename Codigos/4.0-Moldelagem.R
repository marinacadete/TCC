...
pdf("Imagens/envelopes_2x2.pdf", width = 9, height = 9)
par(mfrow = c(2, 2))

# desliga o salvamento interno dos scripts
pdf     <- function(...) invisible(NULL)
png     <- function(...) invisible(NULL)
dev.off <- function(...) invisible(NULL)

fit.model <- modelo_bn_simples_arma
source("Codigos/source/envelope_bn_original.R");    title(main = "(a) BN — arma de fogo")

fit.model <- modelo_bn_simples_cortante
source("Codigos/source/envelope_bn_original.R");    title(main = "(b) BN — cortante")

fit.model <- modelo_pois_simples_enforcamento
source("Codigos/source/envelope_pois_original.R");  title(main = "(c) Poisson — enforcamento")

fit.model <- modelo_pois_simples_mausTratos
source("Codigos/source/envelope_pois_original.R");  title(main = "(d) Poisson — maus-tratos")

# restaura as funções reais e fecha o device combinado
rm(pdf, png, dev.off)
dev.off()
